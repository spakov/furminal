...
namespace Spakov.Terminal {
  public sealed partial class TerminalControl : UserControl {
    ...
    /// state.</remarks>
    internal void ReportFocus() => terminalEngine.SendEscapeSequence([
      (byte) '[',
      (byte) (_hasFocus ? 'I' : 'O')
    ]);
    ...
    internal async void PasteFromClipboard() {
      ...
      if (dataPackageView.Contains(StandardDataFormats.Text)) {
        ...
        if (terminalEngine.BracketedPasteMode) {
        ...
        terminalEngine.SendText(toPaste.ToString());
      }
    }
    ...
    internal void ReportWindowSize() {
      if (terminalEngine.ReportResize) {
        ...
        terminalEngine.SendEscapeSequence(Encoding.ASCII.GetBytes(sizeReport.ToString()));
