...
namespace AnsiProcessor.Output.EscapeSequences.Fe.CSI.SGR {
  ...
  /// <remarks>Sources:
  /// <list type="bullet">
  /// <item><see href="https://en.wikipedia.org/wiki/ANSI_escape_code"/></item>
  /// <item><see href="https://invisible-island.net/xterm/ctlseqs/ctlseqs.html"
  /// /></item>
  ...
  public class SGREscapeSequence : CSIEscapeSequence {
    ...
    public GraphicRendition GraphicRendition {
      get => graphicRendition;
      private set => graphicRendition = value;
    }
    ...
    /// <summary>
    /// The properties that were modified by this <see cref="SGREscapeSequence"/>.
    /// </summary>
    public Properties ModifiedProperties { get; private set; }

    /// <inheritdoc cref="GraphicRendition.Bold"/>
    public bool Bold {
      get => graphicRendition.Bold;

      set {
        graphicRendition.Bold = value;
    ...
    public bool Faint {
      get => graphicRendition.Faint;

      set {
        graphicRendition.Faint = value;
    ...
    public bool Italic {
      get => graphicRendition.Italic;

      set {
        graphicRendition.Italic = value;
    ...
    public bool Underline {
      get => graphicRendition.Underline;

      set {
        graphicRendition.Underline = value;
    ...
    public bool Blink {
      get => graphicRendition.Blink;

      set {
        graphicRendition.Blink = value;
    ...
    public bool Inverse {
      get => graphicRendition.Inverse;

      set {
        graphicRendition.Inverse = value;
    ...
    public bool CrossedOut {
      get => graphicRendition.CrossedOut;

      set {
        graphicRendition.CrossedOut = value;
    ...
    public bool DoubleUnderline {
      get => graphicRendition.DoubleUnderline;

      set {
        graphicRendition.DoubleUnderline = value;
    ...
    public Color ForegroundColor {
      get => graphicRendition.ForegroundColor;

      set {
        graphicRendition.ForegroundColor = value;
    ...
    public Color BackgroundColor {
      get => graphicRendition.BackgroundColor;

      set {
        graphicRendition.BackgroundColor = value;
    ...
    public Color UnderlineColor {
      get => graphicRendition.UnderlineColor;

      set {
        graphicRendition.UnderlineColor = value;
    ...
    public UnderlineStyles UnderlineStyle {
      get => graphicRendition.UnderlineStyle;

      set {
        graphicRendition.UnderlineStyle = value;
    ...
    /// </summary>
    /// <param name="sgrEscapeSequence">An <see cref="Ansi.EscapeSequences.SGR"/>
    /// escape sequence.</param>
    /// <param name="palette">The <see cref="Palette"/> to use for ANSI
    /// colors.</param>
    /// <exception cref="ArgumentException">Thrown when <paramref
    /// name="sgrEscapeSequence"/> is not an <see cref="Ansi.EscapeSequences.SGR"/>
    /// escape sequence.</exception>
    private SGREscapeSequence(string sgrEscapeSequence, Palette palette) : base(sgrEscapeSequence) {
      ...
      // Discard the leading CSI and the trailing control character, then split
      // by ;
      List<string> sequenceParts = [.. sgrEscapeSequence[1..^1].Split(';')];

      // SGR escape sequences can support a ;-separated list of parameters for
      ...
      // But normally, SGR escape sequences can be chained together, separated
      // by ;
      foreach (string sequencePart in sequenceParts) {
        // Check for :-separated parameters, which are an extension for colors
        // and for underline
        if (sequencePart.Contains(':')) {
          ...
          switch (parameters[0]) {
            case Ansi.EscapeSequences.SGR.UNDERLINE:
              if (int.TryParse(parameters[1], out int underlineStyle)) {
                if (underlineStyle is >= ((int) UnderlineStyles.None) and <= ((int) UnderlineStyles.Undercurl)) {
                  Underline = true;
                  UnderlineStyle = (UnderlineStyles) underlineStyle;
                }
              }
          ...
          if (
            ...
            UnderlineStyle = UnderlineStyles.None;
    ...
    /// <summary>
    /// Determines whether an escape sequence is an SGR escape sequence and is
    /// wanted by the <see cref="TerminalCapabilities"/>.
    /// </summary>
    /// <remarks>
    ...
    /// sequence.</param>
    /// <returns><see langword="true"/> if the escape sequence is an SGR escape
    /// sequence or <see langword="false"/> otherwise.</returns>
    internal static bool IsWantedSGREscapeSequence(TerminalCapabilities terminalCapabilities, string rawCSIEscapeSequence) {
      if (!rawCSIEscapeSequence.StartsWith(Ansi.EscapeSequences.Fe.CSI)) return false;
      if (!rawCSIEscapeSequence.EndsWith(Ansi.EscapeSequences.CSI.SGR)) return false;
      ...
      if (sequenceParts.Count > 0) {
        if (terminalCapabilities.ControlCharacters?.EscapeSequences?.CSI?.SGR is not null) {
          if (terminalCapabilities.ControlCharacters.EscapeSequences.CSI.SGR.SGR.Contains(sequenceParts[0])) {
            ...
            // Some extended CSI sequences are separated by : instead
    ...
    /// sequence.</param>
    /// <param name="sgrEscapeSequenceObject">The SGR escape sequence.</param>
    /// <param name="palette">The <see cref="Palette"/> to use for ANSI
    /// colors.</param>
    /// <returns>An <see cref="SGREscapeSequence"/> if the sequence was
    /// processed successfully, or <see langword="null"/> otherwise.</returns>
    ...
    /// <param name="palette"><inheritdoc cref="SGREscapeSequence.SGREscapeSequence"
    /// path="/param[@name='palette']"/></param>
    /// <returns>The <see cref="Color"/> corresponding to <paramref
    /// name="palette"/>, or <see cref="Palette.BrightRed"/> if the sequence
    /// could not be converted into a color.</returns>
